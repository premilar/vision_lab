...
def cd_template_matching(img, template):
	...
	for scale in np.linspace(1.5, .5, 50):
		...
		def box_and_val(temp, orig_w, orig_h):
			res = cv2.matchTemplate(img_canny, temp, cv2.TM_CCORR_NORMED)

			min_val, max_val, min_loc, max_loc = cv2.minMaxLoc(res)
			top_left = max_loc
			bot_right = (int(top_left[0] + orig_w), int(top_left[1] + orig_h))
			return (top_left, bot_right), max_val

		box, val = box_and_val(resized_template, w, h)

		if best_match == None or best_match < val:
			bounding_box = box
			best_match = val
		########### YOUR CODE ENDS HERE ###########

	# print "Max val:", val
	# cv2.rectangle(img_canny2, bounding_box[0], bounding_box[1], (255), 1)
	# cv2.imshow('img canny', img_canny2)
	# cv2.rectangle(img2, bounding_box[0], bounding_box[1], (255), 1)
	# cv2.imshow('img', img2)
